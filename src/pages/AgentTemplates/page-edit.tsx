...
export function UpdateAgentTemplatePage() {
  const {
    handleSubmit,
    register,
    control,
  ...
  const insertAtCursor = (value: string) => {
    ...
    setValue("markdown_desc", newText, { shouldDirty: true });
    return newText;
  };
  ...
  const uploadImage = async (file: File) => {
    ...
    />`.trim();
      ...
      setValue("markdown_desc", final, { shouldDirty: true });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = sectionFields.findIndex((i) => i.fieldId === active.id);
    const newIndex = sectionFields.findIndex((i) => i.fieldId === over.id);
  ...
  const edit = useCallback(
    ...
    [dirtyFields, params.id],
  );

  return (
    <form onSubmit={handleSubmit(edit)} className="flex flex-col">
      <h3 className="text-xl font-extrabold text-gray-900 tracking-tight mb-8">
        Editar template #{params.id}
      </h3>
      <section className="flex mb-5 flex-col space-y-3 bg-neutral-200 p-4">
              ...
              return (
                <div className="w-full grid grid-cols-[1fr_384px] gap-x-2">
                  <Editor
                    ...
                    onChange={(val) => {
                      field.onChange(val ?? "");
                      setValue("chat_demo", val ?? "", { shouldDirty: true });
                    }}
                    options={{
                  ...
                  />
                  <div className="h-120">
                    {safeValue && <DigitizingChatComponent list={safeValue} />}
                  </div>
                </div>
              );
            }}
        ...
        >
          <div className="w-full grid grid-cols-2 gap-x-2">
            ...
            />
            <div className="w-full list bg-[#111111] text-white border border-neutral-600">
              <ReactMarkdown
      ...
      <div className="flex flex-col mb-5 space-y-3 bg-neutral-200 p-4">
        ...
        <div className="w-full">
          ...
          >
            <SortableContext
              items={sectionFields.map((i) => i.fieldId)}
                      ...
                      <div className="flex gap-x-2 w-full p-4 bg-neutral-300">
                        <div className="flex flex-1 flex-col gap-y-2">
                          ...
                          <Field
                            ...
                            errorText={errors.sections?.[index]?.desc?.message}
                            label="Descrição"
                          >
                            <TextareaAutosize
                              style={{ resize: "none" }}
