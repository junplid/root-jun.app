...
export function SelectExtraPackagesComponent({
  isDisabled = false,
  ...props
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await api.get(`/root/extra-packages-options`, {
          headers: { Authorization: cookies.auth_root },
        });
        setList(data.extras.map((c: any) => ({ value: c.id, label: c.name })));
        setLoad(true);
      } catch (error) {
        setLoad(true);
              ...
              return (
                <div className="flex flex-col gap-y-2">
                  <p className="text-slate-700 text-sm">
                    Nenhum pacote extra encontrado!
                  </p>
                </div>
              );
            }}
